...
def set_balance(uid, amt):
    d = load_data(); d[str(uid)] = amt; save_data(d)
...
async def reset(update, context):
    if update.effective_user.id != OWNER_ID:
        ...
        return
    owner_bal = get_balance(OWNER_ID)
    save_data({}); set_balance(OWNER_ID, owner_bal)
    await update.message.reply_text("✅ موجودی ریست شد")
...
async def resetall(update, context):
    if update.effective_user.id != OWNER_ID:
        ...
        return
    owner_bal = get_balance(OWNER_ID)
    save_data({}); set_balance(OWNER_ID, owner_bal); save_stats({})
    await update.message.reply_text("✅ ریست کامل شد")
